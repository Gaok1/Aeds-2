...
void insertionSort(Jogador *array, int tamanho) { // organizar por nome (Não alfabetico) para a pesquisa binaria ser eficiente
   
  Jogador aux;
  for (int i = 1; i < tamanho; i++) {
    aux = array[i];
    int j = i - 1;
    while (j >= 0 && fraseparaInt(array[j].nome) > fraseparaInt(aux.nome)) {
      array[j + 1] = array[j];
      j--;
    }
    array[j + 1] = aux;
...
bool PesquisaBinaria(Jogador* jogador, int *comp,char nome[], int size){
    int esq = 0,dir = size - 1,
    meio;
    while(esq<=dir){
    (*comp)++;
    meio = (esq + dir)/2;
    if(strcmp(nome,jogador[meio].nome) == 0){
        (*comp)++;
        return true;

    }else{
        if((fraseparaInt(jogador[meio].nome) - fraseparaInt(nome) ) < 0){
            esq = meio + 1;
        }else{
            dir = meio - 1;
        }
...
void Documentar(long time,int comp){
    ...
    fprintf(arq,"Matricula: 1441283\tTempo de execução: %li\tComparacoes: %d",tempo,comp);
    fclose(arq);
}
...
int main() {
  ...
  while(true){
    ...
    if (strcmp(entrada, "FIM") == 0) {
      ...
      bool resp = PesquisaBinaria(busca,&comp,entrada,size);
      if(resp == true){
        printf("SIM\n");
      }else{
        printf("NAO\n");
