...
void ShellSort(Jogador *array, int size)
{
  int h = size / 2;
  while (h >= 1)
  {
    for (int i = h; i < size; i++)
    {
      ...
      for (j = i; j >= h; j -= h)
      {
        if (atoi(array[j - h].peso) > atoi(temp.peso))
        {
          array[j] = array[j - h];
        }
        else if (atoi(array[j - h].peso) == atoi(temp.peso))
        {
          if (strcmp(array[j - h].nome, temp.nome) > 0)
          {
            array[j] = array[j - h];
          }
        }
