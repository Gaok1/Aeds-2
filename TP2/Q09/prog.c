...
int OrganizarQuick(Jogador *arr, int esq, int dir) {

  Jogador pivot = arr[dir];
  int i = (esq - 1); // O índice do menor elemento

  for (int j = esq; j <= dir - 1; j++) {
    // Se o elemento atual for menor ou igual ao pivô
    if (OrdemAlfabetica(arr[j].estadoNascimento, pivot.estadoNascimento)) {
      i++; // Incrementa o índice do menor elemento
      trocar(&arr[i], &arr[j]);
    } else if (OrdemAlfabetica(arr[j].estadoNascimento,
                               pivot.estadoNascimento) == 2) {

      if (OrdemAlfabetica(arr[j].nome, pivot.nome)) {
        i++; // Incrementa o índice do menor elemento
        trocar(&arr[i], &arr[j]);
      }
    }
...
void quicksort(Jogador *arr, int esq, int dir) {
  if (esq < dir) {
    // Encontra o índice de particionamento, arr[p] agora está no lugar certo
    int pi = OrganizarQuick(arr, esq, dir);
