...
int OrdemAlfabetica(char *frase1,
                    char *frase2)
{ // retorna true se a frase 1 for antecessora
...
// Função QuickSort
void bubbleSort(Jogador* arr, int size)
{
  ...
  for (int i = 0; i < size - 1; i++)
  {
    ...
    {
      if (atoi(arr[j].anoNascimento) > atoi(arr[j + 1].anoNascimento)) //atoi converte string para int
      {
      ...
      }else if(arr[j].anoNascimento == arr[j + 1].anoNascimento){
        if(OrdemAlfabetica(arr[j].nome, arr[j + 1].nome) == 0){
          Jogador temp = arr[j];
          arr[j] = arr[j + 1];
          arr[j + 1] = temp;
        }
