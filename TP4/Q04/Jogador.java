...
class Arvore { //fazer uma redBlackTree
    ...
    Arvore() {
        this.root = null; // raiz da arvore, na redblack sempre será preta na white black sempre sera branca (false)
    }
    ...
    private void fragmentarNoTipo4(){
            if(IsNoTipo4()){ //se for tipo 4
                this.cor = true; //vai ficar vermelho
                this.esq.cor = false; //filho esquerdo vai ficar preto
                this.dir.cor = false; //filho direito vai ficar preto
                //a fragmentaçao envolve o no subir (vira gemeo do pai) e seus gemeos se tornam seus filhos
            }
        }
   ...
   private void validarInsercao(Jogador jogador) { // Inserir os 3 primeiros na mãozita
        if (this.root == null) {
            ...
            this.root.cor = false; // Define a cor do nó raiz como preto
            ...
            if(jogador.getNome().compareTo(this.root.value.getNome()) < 0){ //se o jogador for menor que a raiz
                this.root.esq = new Node(jogador); //vai ser filho esquerdo
                this.root.esq.cor = true; //vai ser vermelho
            }else{
                this.root.dir = new Node(jogador); //vai ser filho direito
                this.root.dir.cor = true; //vai ser vermelho
            }
            ...
            if(jogador.getNome().compareTo(this.root.value.getNome()) < 0){ //se o jogador for menor que a raiz
                ...
                if(jogador.getNome().compareTo(this.root.dir.value.getNome()) < 0){ //se o jogador for menor que o filho direito
                    Node aux = this.root.dir; //auxiliar recebe o filho direito
                    this.root.esq = new Node(this.root.value); //filho esquerdo recebe o jogador
                    this.root.value = jogador; //raiz recebe o jogador
                }else { //arvore direta para a direita(novo no é maior q filho da direita q é maior q a raiz)
                    this.root.esq = new Node(this.root.value);
    ...
    private void inserir(Node bisavo, Node avo, Node pai, Node i, Jogador jogador){
        if(i == null){  //inserçao do No
            ...
            if(pai.cor == true){ //se o pai for vermelho
                pai.cor = false; //o filho vai ser preto
            }
        }
