...
class HashTable {
    ...
    private int size = 30;
    private int reservaIndice = 21;
    private int sizeReserva = 0; //
    ...
    public boolean Inserir(Jogador x) {
        boolean flag = false;
        int key = hash(x.getAltura());
        if (tabela[key] == null) { // posicao vazia
            tabela[key] = x;
            flag = true;
        } else if (sizeReserva < 9) { // posicao ocupada e ainda tem espaco na reserva
            tabela[sizeReserva + reservaIndice] = x;
            sizeReserva++;
            flag = true;
        } else {
            // nao insere
