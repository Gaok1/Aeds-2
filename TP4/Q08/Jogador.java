...
class HashTable { //Hash com reHash
    private Jogador tabela[];
    private int size = 25;

    public HashTable() {
        this.tabela = new Jogador[size];
    }

    private int hash(Jogador x) {
        return (x.getAltura()%size);
    }
    private int rehash(Jogador x){
        return ((x.getAltura()+1) % size);
    }

    public boolean Inserir(Jogador x) {
        boolean flag = false;
        int key = hash(x);
        if (tabela[key] == null) { // posicao vazia
            tabela[key] = x;
            flag = true;
        } else { // posicao ocupada e ainda tem espaco na reserva
           key = rehash(x);
           if(tabela[key] == null){
            tabela[key] = x;
            flag = true;
    ...
    public boolean pesquisar(Jogador x) {
        if(x != null){
            int key = hash(x);
            if (tabela[key] != null && tabela[key].getNome().equals(x.getNome())) {
                return true;
            } else {
                key = rehash(x);
                if (tabela[key] != null && tabela[key].getNome().equals(x.getNome())) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            if (entrada.equals("FIM")) {
                ...
                Jogador aux = getJogadorByName(entrada, time);
                MyIO.println(entrada);
                if (tabela.pesquisar(aux)) {
                    entrada += " SIM";
                } else {
                    entrada += " NAO";
                }
